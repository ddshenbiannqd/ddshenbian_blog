$(function() {
	...
	$('.img3').click(function() {
		...
		if (sysDate.getMonth() + 1 >= deadLine) {
			...
			if (!isLogin) {
				// var form = $("<form></form>");
				// var action = "http://m.fullrong.com/wap/logoutSuccessDzp?his=/wap/theme/wapdial/index.html"
				// form.attr('action',action);
				// form.attr('method','get');  
				// form.appendTo('body');
				// form.css('display','none');
				// form.submit();
				window.location.href="http://m.fullrong.com/wap/logoutSuccessDzp?his=/wap/theme/wapdial/index.html"
				// window.location.href = "http://www.fullrong.com/mobile/loginTimeout";
				isture = false;
			} else { //登录了就执行下面
	...
	function drawPrize() {
		$.ajax({
			type: "get",
			url: "http://m.fullrong.com/wap/draw/drawprize/1", //URL
			// url: "data3.json", //URL
			dataType: "json",
	...
	function getPrizeList(alllistPage, allPerPage) {
		$.ajax({
			type: "get",
			url: "http://m.fullrong.com/wap/draw/drawinfo/1/" + alllistPage + "/" + allPerPage, //URL
			// url: "data1.json", //URL
			dataType: "json",
			success: function(jsondata) {
	...
	function getMyPrizeList(mylistPage, myPerPage) {
		$.ajax({
			type: "get",
			url: "http://m.fullrong.com/wap/draw/customerDrawInfo/1/" + mylistPage + "/" + myPerPage, //URL
			// url: "data2.json", //URL
			dataType: "json",
			success: function(jsondata) {
				if (jsondata.code == 3) {
	...
	function getSysTime() {
		$.ajax({
			type: "get",
			url: "http://m.fullrong.com/wap/sysCurrentTime", //URL
			// url: "data4.json", //URL
			dataType: "json",
			success: function(jsondata) {
